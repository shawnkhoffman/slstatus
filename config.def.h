...
static const struct arg args[] = {
	...
	{ wifi_essid,            "   %s ",        "wlan0" },       // WiFi SSID
	{ wifi_perc,             "(%s%%)  ",       "wlan0" },       // WiFi signal strength (percent)
	{ netspeed_rx,           "%sB/s ",        "wlan0"    },    // Download speed
	{ netspeed_tx,           "%sB/s]          ", "wlan0"    }, // Upload speed
	{ cpu_perc,              "[ CPU %s%% ",   NULL	},          // CPU usage (percent)
	{ run_command,           "(%s),",          "sensors coretemp-isa-0000 | awk '/+(.*)/ {print $3}' | awk 'NR==2'"}, // CPU temperature
	{ ram_perc,              "  RAM %s%%]     ", NULL	},      // RAM usage (percent)
	{ run_command,           "墳%4s  ",         "awk -F'[][]' '/Left:/ { print $2 }' <(amixer sget Master)"}, // System volume (percentage)
